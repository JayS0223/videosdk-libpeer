...
static void peer_connection_outgoing_rtp_packet(uint8_t *data, size_t size, void *user_data) {
 ...
 int sent = agent_send(&pc->agent, data, size);
  LOGI("peer_connection: Sent RTP packet of size %zu, agent_send returned %d", size, sent);
}
...
//   LOGD("DTLS: Starting DTLS recv with len=%d", (int)len);

//   if (pc->agent_ret > 0 && pc->agent_ret <= len) {
//     LOGD("DTLS: Using buffered agent data of size %d", pc->agent_ret);
...
//     if (ret > 0) {
//       LOGD("DTLS: Received %d bytes from agent", ret);
//       break;
//     } else if (ret == 0) {
//       LOGD("DTLS: No data received (ret=0), attempt %d", recv_max + 1);
//     } else {
//       LOGW("DTLS: agent_recv returned error: %d", ret);
//     }

//     recv_max++;
//   }

//   if (recv_max >= MAX_RECV) {
//     LOGW("DTLS: Max receive attempts (%d) reached with no data", MAX_RECV);
//   }
...
int peer_connection_loop(PeerConnection *pc) {
  ...
  switch (pc->state) {
    ...
    case PEER_CONNECTION_CONNECTED:
      ...
      else{if (KEEPALIVE_CONNCHECK > 0 && (ports_get_epoch_time() - pc->agent.binding_request_time) > KEEPALIVE_CONNCHECK) {
