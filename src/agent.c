...
LOGD("agent_process_stun_request:");
  ...
  switch (stun_msg->stunmethod) {
    case STUN_METHOD_BINDING:
      if (stun_msg_is_valid(stun_msg->buf, stun_msg->size, agent->local_upwd) == 0) {
        ...
        LOGD("before stun request in STUN_METHOD_BINDING:%d", stun_msg->stunmethod);
        agent->binding_request_time = ports_get_epoch_time();
        LOGD("after stun request in STUN_METHOD_BINDING:%d", stun_msg->stunmethod);
      }
      break;
    default:
      break;
